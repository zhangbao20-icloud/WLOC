...
> **旧仓库迁移提示：** 如果设备上此前安装的是 `zhangbao20-sina/wloc` 版本，请不要只点“刷新/更新远程资源”，因为客户端保存的远程源地址仍可能指向旧仓库。请删除旧的 WLOC 远程模块后，通过上方当前一键安装入口重新导入一次，或直接使用本仓库对应的 GitHub RAW 地址重新添加。确认模块信息中的仓库、图标和脚本地址均为 `zhangbao20-icloud/WLOC` 后，再删除旧仓库。
